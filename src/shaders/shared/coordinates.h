vec4 screenToEyeSpaceOrthographic(vec3 windowSpace)
{
	vec3 ndcPos;
	ndcPos.xy = (windowSpace.xy / viewport.zw) * 2 - vec2(1);
	ndcPos.z = (2.0 * windowSpace.z - gl_DepthRange.near - gl_DepthRange.far) / (gl_DepthRange.far - gl_DepthRange.near);

	// orthographic
	vec4 eyePos = cameraInverseProj * vec4(ndcPos, 1);
	return eyePos;
}

vec4 screenToEyeSpacePerspective(vec3 windowSpace)
{
...
vec3 screenToWorldSpacePerspective(vec3 windowSpace)
{
	vec4 eyePos = screenToEyeSpacePerspective(windowSpace);
