...
// directions.
vec3 sampleBSDF_Lambertian(in vec3 albedo, 
						   in vec3 lsWo, 
...
vec4 evaluateBSDF_Lambertian(in vec3 albedo, in vec3 lsWo, in vec3 lsWi)
{
	// This follows the same logic as the cosine-weighted hemiphere sampling.
	float pdf = lsWi.y / PI;
	vec3 f = albedo / PI;
	return vec4(f,pdf);
}
